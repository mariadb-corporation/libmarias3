...
static uint8_t build_request_uri(CURL *curl, const char *base_domain, const char *bucket, const char *object, const char *query)
{
  ...
  if (query)
  {
    if (path_parts + strlen(domain) + strlen(bucket) + strlen(object) + strlen(query) >= MAX_URI_LENGTH - 1)
    {
      return MS3_ERR_URI_TOO_LONG;
    }
    snprintf(uri_buffer, MAX_URI_LENGTH - 1, "https://%s.%s%s?%s", bucket, domain, object, query);
  }
  else
  {
    if (path_parts + strlen(domain) + strlen(bucket) + strlen(object) >= MAX_URI_LENGTH - 1)
    {
      return MS3_ERR_URI_TOO_LONG;
    }
    snprintf(uri_buffer, MAX_URI_LENGTH - 1, "https://%s.%s%s", bucket, domain, object);
  }
...
*/
static uint8_t generate_request_hash(uri_method_t method, const char *path, const char *query, char *post_hash, struct curl_slist *headers, char *return_hash)
{
    ...
    uri_method_t method, const char *bucket, put_buffer_st *post_data)
{
  ...
  snprintf(headerbuf, sizeof(headerbuf), "x-amz-content-sha256:%.*s", 64, post_hash);
  headers = curl_slist_append(headers, headerbuf);
  ...
  strftime(headerbuf + offset, sizeof(headerbuf) - offset, "%Y%m%dT%H%M%SZ", gmtime(&now));
  headers = curl_slist_append(headers, headerbuf);

  // Builds the request hash
  ret = generate_request_hash(method, object, query, post_hash, headers, sha256hash);
  if (ret)
  {
    return ret;
  }
...
uint8_t execute_request(ms3_st *ms3, command_t cmd, const char *bucket, const char *object, const char *filter, const uint8_t *data, size_t data_size, void *ret_ptr)
{
  ...
  res = build_request_headers(curl, headers, ms3->base_domain, ms3->region, ms3->s3key, ms3->s3secret, path, query, method, bucket, &post_data);
