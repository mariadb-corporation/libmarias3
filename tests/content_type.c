...
int main(int argc, char *argv[])
{
  ...
  ms3_set_content_type(ms3, "text/plain");
  res = ms3_put(ms3, s3bucket, "test/content_type.txt",
  ...
  while (list_it)
  {
    if (!strncmp(list_it->key, "test/content_type.txt", 21))
    {
      found = true;
      break;
    }
  ...
  res = ms3_get(ms3, s3bucket, "test/content_type.txt", &data, &length);
  ASSERT_EQ_(res, 0, "Result: %u", res);
  ASSERT_EQ(length, 26);
  ASSERT_STREQ((char *)data, test_string);

  content_type_in = ms3_get_content_type(ms3);

  ASSERT_STREQ(content_type_in, "text/plain");

  for (i = 0; i <= 3; i++)
  {
    res = ms3_status(ms3, s3bucket, "test/content_type.txt", &status);
  ...
  res = ms3_delete(ms3, s3bucket, "test/content_type.txt");
  ASSERT_EQ_(res, 0, "Result: %u", res);
  ms3_free(data);
  res = ms3_get(ms3, s3bucket, "test/content_type.txt", &data, &length);
